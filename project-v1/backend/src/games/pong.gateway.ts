...
export class PongGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
	...
	/* Invite room */
	createInvitedUser(id: string, username: string) {
	...
	roomAlreadyExists(senderId: string, receiverId: string) {
	...
	async createInviteRoom(sender: User, receiverId: string) {
		this.logger.log("Create new Invite room");

		const firstPlayer: User = this.createInvitedUser(sender.id, sender.username);
		const receiverData = await this.usersService.findUserById(String(receiverId));
		const secondPlayer: User = this.createInvitedUser(receiverData.id, receiverData.firstName);
	...
	async saveGame(room: Room, currentTimestamp: number){
		let winnerId: string, loserId: string, winnerScore: number, loserScore: number;
		...
		const winner = await this.usersService.findUserById(String(winnerId));
		const loser = await this.usersService.findUserById(String(loserId));
